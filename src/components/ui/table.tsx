...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    ...
    >
      <table
        data-slot="table"
        className={cn("w-full min-w-max caption-bottom text-[12px] sm:min-w-0", className)}
        {...props}
      />
    </div>
  )
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-8 px-2 text-left align-middle text-[10.5px] font-semibold uppercase whitespace-nowrap text-muted-foreground [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
