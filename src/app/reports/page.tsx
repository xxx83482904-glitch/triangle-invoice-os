...
export default async function ReportsPage() {
  ...
  const months = Array.from(
    new Set([
      ...data.issuedInvoices.map((invoice) => monthKey(invoice.issueDate)),
      ...data.payments.map((payment) => monthKey(payment.paymentDate)),
      ...data.receivedInvoices.map((invoice) => monthKey(invoice.dueDate)),
    ]),
  ).sort();
  ...
  return (
    <AppShell>
      <PageHeader title="レポート" description="月別、案件別、クライアント別、支払先別の数字をCSVに落とせる形で確認します。">
        <Button asChild variant="outline"><Link href="/api/export/projects">案件別CSV</Link></Button>
      </PageHeader>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>{months.map((month) => {
                const issued = data.issuedInvoices.filter((invoice) => monthKey(invoice.issueDate) === month);
                const income = data.payments.filter((payment) => payment.type === "INCOME" && monthKey(payment.paymentDate) === month);
                const received = data.receivedInvoices.filter((invoice) => monthKey(invoice.dueDate) === month);
                const expense = data.payments.filter((payment) => payment.type === "EXPENSE" && monthKey(payment.paymentDate) === month);
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>{data.projects.filter((project) => !project.deletedAt).map((project) => {
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>{data.clients.filter((client) => !client.deletedAt).map((client) => {
                const invoices = data.issuedInvoices.filter((invoice) => invoice.clientId === client.id && !invoice.deletedAt);
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>{data.vendors.filter((vendor) => !vendor.deletedAt).map((vendor) => {
                const invoices = data.receivedInvoices.filter((invoice) => invoice.vendorId === vendor.id && !invoice.deletedAt);
